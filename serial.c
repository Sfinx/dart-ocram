...
#define IOMUXC_UART1_IPP_UART_RXD_MUX_SELECT_INPUT	__REG(0x20E0624)

#define UART_PHYS			(0x02020000)

#define UART_URXD			__REG(UART_PHYS)
...
void dbg_init(void)
{
	IOMUXC_SW_MUX_CTL_PAD_CSI0_DATA10 = 0x0;
	IOMUXC_SW_MUX_CTL_PAD_CSI0_DATA11 = 0x0;
	IOMUXC_UART1_IPP_UART_RXD_MUX_SELECT_INPUT = 0x03;
...
char dbg_getc(void)
{
 while (!(UART_USR2 & USR2_RDR));
 return (UART_URXD & 0xFF);
}
